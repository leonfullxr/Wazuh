{{/*
Manager ossec.conf. Body is upstream wazuh-kubernetes master.conf for this
chart's appVersion, unchanged except for a templated <cluster> block. Re-diff
on every Wazuh version bump:

  curl -sL https://raw.githubusercontent.com/wazuh/wazuh-kubernetes/v<ver>/wazuh/wazuh_managers/wazuh_conf/master.conf
...
<ossec_config>
  ...
  <cluster>
    <name>{{ $.Values.manager.clusterName }}</name>
    ...
    <nodes>
        <node>{{ include "wazuh.managerMasterFqdn" $ }}</node>
    </nodes>
    <hidden>no</hidden>
    <disabled>no</disabled>
  </cluster>
...
{{- with $.Values.manager.extraOssecConfig }}
