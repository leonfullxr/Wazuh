...
2. The ALB health-checks the Wazuh API (55000), which correctly returns HTTP 401 to an unauthenticated probe. The ALB treats 401 as unhealthy, so the gate never opens, so the STS never becomes Ready.
