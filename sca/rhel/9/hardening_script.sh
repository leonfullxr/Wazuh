#!/bin/bash
# CIS RHEL 9 v2.0.0 Hardening Script - NATIVE RHEL 9 & WAZUH OPTIMIZED
#
...
apply_kernel_modules() {
    ...
    # RHEL 9 CIS v2.0.0 recommended disabled modules
    MODULES=(cramfs freevxfs hfs hfsplus jffs2 squashfs udf usb-storage firewire-core tipc dccp sctp rds atm)
...
apply_ssh_config() {
    ...
    # RHEL 9 Crypto Policy
    mkdir -p /etc/crypto-policies/policies/modules/
    cat > /etc/crypto-policies/policies/modules/CIS-RHEL9.pmod << 'EOF'
cipher@SSH = -CHACHA20-POLY1305 -AES-128-CBC -AES-256-CBC -3DES-CBC
mac@SSH = -HMAC-SHA1 -HMAC-SHA1-96 -UMAC-64-ETM -UMAC-64
EOF
    update-crypto-policies --set DEFAULT:CIS-RHEL9 2>/dev/null || update-crypto-policies --set DEFAULT 2>/dev/null || true
...
apply_pam_config() {
    ...
    # Select the profile with valid RHEL 9 flags
...
apply_file_permissions() {
    ...
    chmod 0000 /etc/security/opasswd
...
apply_misc_fixes() {
    ...
    sed -i 's/OPTIONS=" -u chrony"/OPTIONS="-u chrony"/' /etc/sysconfig/chronyd 2>/dev/null || true
