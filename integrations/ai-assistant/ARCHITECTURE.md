...
- **The chat lives inside the Wazuh Dashboard.** The Dashboards Assistant
  plugin to OpenSearch ML Commons to an HTTP connector to our
...
1. **Lane 0 (D40):** one embedding matches the question to a curated bilingual
   template, which becomes typed IR, then executes with no model in the loop;
   a near-miss just under threshold becomes a few-shot hint instead of being
   discarded.
...
- **Two phases (D20):** the model calls a `propose_*` tool (typed schema +
  preview), which gives a proposal card, nothing executed. A human confirms;
  only then does a per-tier executor credential (never the model, D35) run it.
