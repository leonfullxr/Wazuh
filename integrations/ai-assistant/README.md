...
The current architecture and end-to-end turn are drawn in [`diagrams/wazuh-ai-v3-gateway.drawio`](diagrams/wazuh-ai-v3-gateway.drawio) (topology) and [`diagrams/wazuh-ai-v3-workflow.drawio`](diagrams/wazuh-ai-v3-workflow.drawio) (one turn, as built). Those draw.io sources are current; their PNG exports are pending a manual export pass. The eight PNGs in the next section are the **original self-hosted PoC (v1/v2)** and remain accurate for that layer — the veracity ladder, the four checks, the local harness — which every later phase builds on unchanged.
