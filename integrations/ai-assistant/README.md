...
- **Identity, no external IdP (V3.6).** The auth-shim verifies analyst credentials against each environment's own indexer (`_plugins/_security/authinfo`) and mints the short-lived turn JWT — Keycloak was the earlier stand-in and has been removed. Direct edges keep per-user identity (D11); the dashboard edge runs as a read-only per-environment principal (D42), disclosed in every answer's label.
- **Write actions by design (V3.5/V3.8), not read-only.** The model proposes typed actions (create dashboard, restart agent, active response) that a human confirms — via `/v1/actions` or a conversational "yes"/"no" — before a tiered executor credential (never the model) runs them. High-risk actions require echoing the target. See [`ARCHITECTURE-V3.5-ACTIONS.md`](ARCHITECTURE-V3.5-ACTIONS.md) and [`ARCHITECTURE-V3.8-UX.md`](ARCHITECTURE-V3.8-UX.md).
- **Broader context and coverage (V3.4/V3.7):** a separate typed path for `wazuh-states-vulnerabilities-*`, environment tools (agents, index health, dashboards), in-cluster ML Commons embeddings, and per-environment context cards — all under the same "query, don't embed" rule.

The current architecture and end-to-end turn are drawn in [`diagrams/wazuh-ai-v3-gateway.drawio`](diagrams/wazuh-ai-v3-gateway.drawio) (topology) and [`diagrams/wazuh-ai-v3-workflow.drawio`](diagrams/wazuh-ai-v3-workflow.drawio) (one turn, as built), with an icon-forward version of both in [`diagrams/wazuh-ai-v3-icons.drawio`](diagrams/wazuh-ai-v3-icons.drawio). Those draw.io sources are current; their PNG exports are pending a manual export pass. The eight PNGs in the next section are the **original self-hosted PoC (v1/v2)** and remain accurate for that layer — the veracity ladder, the four checks, the local harness — which every later phase builds on unchanged.

## Architecture diagrams

The eight diagrams below are the original self-hosted PoC (v1/v2) at a glance, exported from an editable draw.io source at [`diagrams/wazuh-ai-poc-architecture.drawio`](diagrams/wazuh-ai-poc-architecture.drawio). The D-tags in the labels are defined in [section 9](#9-design-decision-tags). A second deck, [`diagrams/wazuh-ai-enhancements.drawio`](diagrams/wazuh-ai-enhancements.drawio), shows the as-built enhancement pass (scope classifier, near-miss few-shot, knowledge tool, prompt caching, the AMD local test harness) with the measured golden-set results. For the **current v3 architecture** (dashboard-embedded, multi-environment, with actions) see the v3 sources linked in [the section above](#where-the-architecture-is-now-v3-as-built); the PNGs here predate that phase and depict the veracity core it still builds on.
