...
1. Go to Explore, then Visualize, then Create visualization.
2. Choose Data Table over the `wazuh-alerts-*` index pattern.
3. Add a bucket, then Split rows, then Aggregation Terms, then Field `rule.id`, then Update.
...
1. In the dashboard go to Server Management, then Rules, search for the rule ID (e.g. `60106`) and copy the original rule definition.
2. Open Custom rules, then `local_rules.xml`, paste the rule, set `level="0"` and add `overwrite="yes"`:
...
- Raise `events_per_second` gradually (steps of ~100, up to 1000) and observe whether the buffer-full alerts stop, since higher rates increase agent and manager load.
